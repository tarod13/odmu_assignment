...
from src.algs import DynamicProphetSolver
...
def run_evaluation(
    ...
    # Create game
    game = ProphetGame(means, vars, limits, init_prophet=True)
    prophet_value = game.estimate_prophet_value(int(prophet_samples), k)[0]
    
    # Create solver and set threshold 
    ALG_solver = DynamicProphetSolver(n=n, k=k)
    ALG_solver.set_threshold_dict(game.prophet_dict)
